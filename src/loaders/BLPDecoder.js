...
// Mirrors WMVx/ddslib.cpp's RGB565 expansion. WMVx performs integer
// bit expansion/truncation rather than floating-point rounding.
...
function decodeDXT1Block(src, offset, alphaMode) {
  ...
  // Exact integer interpolation used by WMVx/ddslib.cpp.
  if (c0 > c1 || !alphaMode) {
    colors[2][0] = Math.floor((2 * r0 + r1) / 3);
    colors[2][1] = Math.floor((2 * g0 + g1) / 3);
    colors[2][2] = Math.floor((2 * b0 + b1) / 3);
    colors[3][0] = Math.floor((r0 + 2 * r1) / 3);
    colors[3][1] = Math.floor((g0 + 2 * g1) / 3);
    colors[3][2] = Math.floor((b0 + 2 * b1) / 3);
  } else {
    colors[2][0] = Math.floor((r0 + r1) / 2);
    colors[2][1] = Math.floor((g0 + g1) / 2);
    colors[2][2] = Math.floor((b0 + b1) / 2);
...
function decodeDXT5Block(src, offset) {
  ...
  const values = [a0, a1];
  if (a0 > a1) {
    for (let i = 1; i <= 6; i++) values.push(Math.floor(((7 - i) * a0 + i * a1) / 7));
  } else {
    for (let i = 1; i <= 4; i++) values.push(Math.floor(((5 - i) * a0 + i * a1) / 5));
    values.push(0, 255);
  }
  let bits = 0n;
...
function decodePalette(data, palette, width, height, alpha) {
  ...
  for (let i = 0; i < width * height; i++) {
    const p = data[i] * 4;
    const d = i * 4;
    // WMVx swaps B/R for the palette entries before uploading RGBA.
    out[d] = palette[p + 2];
    out[d + 1] = palette[p + 1];
    out[d + 2] = palette[p];
...
export class BLPDecoder {
  decode(input) {
    ...
    if (magic === 'BLP2') {
      // Exact WMVx BLPHeader layout:
      // version @ 0x04, colorEncoding @ 0x08, alphaSize @ 0x09,
      // preferredFormat @ 0x0a, hasMips @ 0x0b.
      const version = data.readUInt32LE(4);
      const colorEncoding = data[8];
      const alphaSize = data[9];
      const preferredFormat = data[10];
      const hasMips = data[11];
      if (version !== 1) throw new Error(`Unsupported BLP2 version: ${version}`);
      ...
      if (colorEncoding === 1) {
        ...
        const alpha = alphaSize === 1 ? mip.subarray(width * height) : null;
        pixels = decodePalette(mip, palette, width, height, alpha);
        if (alphaSize === 8) {
          const alpha8 = mip.subarray(width * height, width * height * 2);
          for (let i = 0; i < width * height; i++) pixels[i * 4 + 3] = alpha8[i] ?? 255;
        }
      } else if (colorEncoding === 2) {
        // WMVx selects DXT3 for alphaSize 4/8, then DXT5 when the
        // preferred pixel format is PIXEL_DXT5 (7).
        const format = alphaSize === 8 && preferredFormat === 7
          ? 5
          : alphaSize === 8 || alphaSize === 4
            ? 3
            : 1;
        pixels = decodeDXT(mip, width, height, format);
      } else if (colorEncoding === 3 || colorEncoding === 4) {
        pixels = decodeRawBGRA(mip, width, height);
      } else {
        throw new Error(`Unsupported BLP2 color encoding: ${colorEncoding}`);
      }
      ...
      return {
        ...
        channels: 4,
        pixels,
        format: { magic, version, colorEncoding, alphaSize, preferredFormat, hasMips },
      };
    }
    ...
    if (compression === 1) {
      ...
      const alpha = alphaDepth === 1 ? mip.subarray(width * height) : null;
      pixels = decodePalette(mip, palette, width, height, alpha);
