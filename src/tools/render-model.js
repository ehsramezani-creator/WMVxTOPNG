...
function usage() {
  console.error('Usage: node src/tools/render-model.js <M2> [output.png] [modelsRoot] [dbRoot]');
  process.exit(2);
}
...
async function collectFiles(root) {
  ...
  async function walk(dir) {
    for (const entry of await fs.readdir(dir, { withFileTypes: true })) {
      ...
      if (entry.isDirectory()) await walk(full); else out.set(normalize(path.relative(root, full)), full);
    }
  }
...
function findDb(root) {
  if (!root) return null;
  const candidates = [
    path.join(root, 'DBFilesClient', 'CharSections.dbc'),
    path.join(root, 'dbfilesclient', 'CharSections.dbc'),
    path.join(root, 'CharSections.dbc')
  ];
  return candidates[0];
}
...
const characterResolver = new CharacterTextureResolver({ decoder, files });
const dbPath = findDb(path.resolve(dbRoot));
const characterTexture = await characterResolver.resolve(m2, { dbPath });
...
async function decodeTexture(name) {
  ...
  const texturePath = files.get(key) ?? files.get(key.endsWith('.blp') ? key : `${key}.blp`);
...
let textureStats = { referenced: 0, found: 0, decoded: 0, characterResolved: false, missing: [] };
...
console.log(JSON.stringify({
  model: m2.name, version: m2.version, vertices: model.vertices.length, triangles: model.indices.length / 3,
  skin: path.basename(m2.skin.filePath ?? ''), textures: m2.textures.length,
  characterTexture: characterTexture.enabled ? { identity: characterTexture.identity, layers: characterTexture.layers?.length ?? 0, missingBase: characterTexture.missingBase ?? null } : characterTexture,
  textureStats, output: path.resolve(outputPath)
}, null, 2));
